        ...
        <div className="w-full max-w-xl bg-white shadow-lg relative ml-auto h-screen">
          <div className="overflow-auto p-6 h-[calc(100vh-135px)]">
            ...
            <div className="divide-y mt-4">
              {cart && cart.map((item) => (
                <div key={item.id} className="flex items-start justify-between gap-4 py-8">
                  <div className="flex max-sm:flex-col gap-6">
                  <div className="h-40 bg-gray-100 p-4 rounded">
                    <img src={item.image} className="w-full h-full object-contain shrink-0" alt={item.name} />
                  </div>
                  <div>
                    <p className="text-md font-bold text-[#333]">{item.name}</p>
                    <p className="text-gray-400 text-xs mt-1">1 Item</p>
                    <h4 className="text-xl font-bold text-[#333] mt-4">{item.price}</h4>
                    <div className="mt-6">
          ...
          <div className="p-6 absolute bottom-0 w-full border-t bg-white">
            <ul className="text-[#333] divide-y">
              <li className="flex flex-wrap gap-4 text-md font-bold">Subtotal <span className="ml-auto">$69.5</span></li>
            </ul>
